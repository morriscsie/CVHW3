...
def do_coco_evaluation(
    ...
    if "bbox" in iou_types:
        ...
        Dict, coco_results["bbox"] = prepare_for_coco_detection(predictions, dataset)
    ...
    for iou_type in iou_types:
        with tempfile.NamedTemporaryFile() as f:
            ...
            # modification comes from https://github.com/facebookresearch/maskrcnn-benchmark/issues/524 
            for catId in dataset.coco.getCatIds():
                print(f"Category: {catId} {dataset.coco.cats[catId]}")
...
def prepare_for_coco_detection(predictions, dataset):
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        for k, box in enumerate(boxes):
            Labels.append(mapped_labels[k])
            Boxes.append(box)
            Scores.append(scores[k])
            with open("./test.coco.json","r") as json_file:
                dicts = json.load(json_file)
                for dict in dicts["images"]:
                    if dict["id"] == original_id:
                        img_path = dict["file_name"]
        Dict[img_path] = {"boxes":Boxes, "labels":Labels, "scores":Scores}


    return Dict,coco_results
